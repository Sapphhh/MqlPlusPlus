...
bool MQL5::Metatrader5::Login(size_t login, const char* password, const char* server_name, size_t timeout)
{
	...
	if (timeout != 0)
	{
		...
		py_value = PyLong_FromUnsignedLongLong(timeout);
		PyDict_SetItem(py_kwargs, py_key, py_value);
...
bool MQL5::Metatrader5::AccountInfo(MqlAccountInfo& account_info)
{
	...
	account_info.limit_orders = PyLong_AsInt(ret);
	...
	account_info.currency_digits = PyLong_AsInt(ret);

	ret = PyTuple_GetItem(py_ret_value, 9);
	account_info.fifo_close = (bool)PyLong_AsLong(ret);
	...
	PyCpyStr(account_info.name, tmp_size);

	ret = PyTuple_GetItem(py_ret_value, 25);
	tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
	PyCpyStr(account_info.server, tmp_size);

	ret = PyTuple_GetItem(py_ret_value, 26);
	tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
	PyCpyStr(account_info.currency, tmp_size);

	ret = PyTuple_GetItem(py_ret_value, 27);
	tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
	PyCpyStr(account_info.company, tmp_size);
...
bool MQL5::Metatrader5::GetLastError(MqlErrorInfo& error_info)
{
	...
	PyCpyStr(error_info.desc, tmp_size);
...
long MQL5::Metatrader5::SymbolsTotal()
{
	...
	long ret = PyLong_AsLong(py_ret_value);
...
long MQL5::Metatrader5::SymbolsGet(const char* group, MqlSymbolInfoContainer& symbols)
{
	...
	symbols.mql_object.elem_count = static_cast<long>(PyTuple_GET_SIZE(py_ret_value));
	...
	for (Py_ssize_t i = 0; i < py_ret_len; ++i)
	{
		...
		symbol_info.trade_stops_level = PyLong_AsInt(ret);

		ret = PyTuple_GetItem(ret_tuple, 20);
		symbol_info.trade_freeze_level = PyLong_AsInt(ret);
		...
		symbol_info.expiration_mode = PyLong_AsInt(ret);

		ret = PyTuple_GetItem(ret_tuple, 26);
		symbol_info.filling_mode = PyLong_AsInt(ret);

		ret = PyTuple_GetItem(ret_tuple, 27);
		symbol_info.order_mode = PyLong_AsInt(ret);
		...
		tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
		PyCpyStr(symbol_info.basis, tmp_size);

		ret = PyTuple_GetItem(ret_tuple, 84);
		tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
		PyCpyStr(symbol_info.category, tmp_size);

		ret = PyTuple_GetItem(ret_tuple, 85);
		tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
		PyCpyStr(symbol_info.currency_base, tmp_size);

		ret = PyTuple_GetItem(ret_tuple, 86);
		tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
		PyCpyStr(symbol_info.currency_profit, tmp_size);

		ret = PyTuple_GetItem(ret_tuple, 87);
		tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
		PyCpyStr(symbol_info.currency_margin, tmp_size);

		ret = PyTuple_GetItem(ret_tuple, 88);
		tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
		PyCpyStr(symbol_info.bank, tmp_size);

		ret = PyTuple_GetItem(ret_tuple, 89);
		tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
		PyCpyStr(symbol_info.description, tmp_size);

		ret = PyTuple_GetItem(ret_tuple, 90);
		tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
		PyCpyStr(symbol_info.exchange, tmp_size);

		ret = PyTuple_GetItem(ret_tuple, 91);
		tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
		PyCpyStr(symbol_info.formula, tmp_size);

		ret = PyTuple_GetItem(ret_tuple, 92);
		tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
		PyCpyStr(symbol_info.isin, tmp_size);

		ret = PyTuple_GetItem(ret_tuple, 93);
		tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
		PyCpyStr(symbol_info.name, tmp_size);

		ret = PyTuple_GetItem(ret_tuple, 94);
		tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
		PyCpyStr(symbol_info.page, tmp_size);

		ret = PyTuple_GetItem(ret_tuple, 95);
		tmp_str = PyUnicode_AsUTF8AndSize(ret, &tmp_size);
		PyCpyStr(symbol_info.path, tmp_size);
	}
...
long MQL5::Metatrader5::CopyRatesFrom(const char* symbol, ENUM_TIMEFRAMES timeframe, datetime date_from, ulong count, MqlRatesContainer& rates)
{
...
long MQL5::Metatrader5::CopyRatesFromPos(const char* symbol, ENUM_TIMEFRAMES timeframe, int bar_from, ulong count, MqlRatesContainer& rates)
{
	...
	py_value = PyLong_FromUnsignedLongLong(count);
	...
	rates.mql_object.object = py_ret_value;
	rates.mql_object.elem_count = static_cast<long>(PyArray_SIZE(py_ret_value));
...
long MQL5::Metatrader5::CopyRatesRange(const char* symbol, ENUM_TIMEFRAMES timeframe, datetime date_from, datetime date_to, MqlRatesContainer& rates)
{
	...
	py_value = PyLong_FromUnsignedLongLong(date_from);
	PyTuple_SetItem(py_args, 2, py_value);

	py_value = PyLong_FromUnsignedLongLong(date_to);
	PyTuple_SetItem(py_args, 3, py_value);
...
long MQL5::Metatrader5::CopyTicksFrom(const char* symbol, datetime date_from, ulong count, long flags, MqlTickContainer& ticks)
{
	...
	ticks.mql_object.elem_count = static_cast<long>(PyArray_SIZE(py_ret_value));
...
bool MQL5::Metatrader5::OrderSend(const MqlTradeRequest& request, MqlTradeResult& result)
{
	...
	py_value = PyLong_FromUnsignedLongLong(request.deviation);
	PyDict_SetItemString(py_dict, "deviation", py_value);
	Py_XDECREF(py_value);

	py_value = PyLong_FromUnsignedLongLong(request.order);
	PyDict_SetItemString(py_dict, "order", py_value);
	Py_XDECREF(py_value);

	py_value = PyLong_FromUnsignedLongLong(request.position);
	PyDict_SetItemString(py_dict, "position", py_value);
	Py_XDECREF(py_value);

	py_value = PyLong_FromUnsignedLongLong(request.position_by);
	...
	py_value = PyLong_FromUnsignedLongLong(request.expiration);
	PyDict_SetItemString(py_dict, "expiration", py_value);
	Py_XDECREF(py_value);

	py_value = PyFloat_FromDouble(request.stoplimit);
	PyDict_SetItemString(py_dict, "stoplimit", py_value);
	Py_XDECREF(py_value);
	...
	result.retcode = PyLong_AsUnsignedLong(py_item);
	...
	PyCpyStr(result.comment, tmp_size);
...
long MQL5::Metatrader5::OrdersGet(MqlOrderInfoContainer& orders)
{
	...
	orders.size = static_cast<long>(orders_count);
...
long MQL5::Metatrader5::OrdersGetTickets(const char* symbol, ulong*& orders)
{
	static PyObject*& py_fn = PY_FUNC_TABLE[21].first;
	static PyCFunctionWithKeywords& fn = PY_FUNC_TABLE[21].second;
	...
	orders = new ulong[orders_count];

	for (int i = 0; i < orders_count; ++i)
	{
		PyObject* py_order_tuple = PyTuple_GetItem(py_ret_value, i);
		orders[i] = PyLong_AsUnsignedLongLong(PyTuple_GetItem(py_order_tuple, 0));
	}
...
long MQL5::Metatrader5::PositionsGet(MqlPositionInfoContainer& positions_info)
{
	...
	positions_info.size = static_cast<long>(positions_count);
...
long MQL5::Metatrader5::HistoryOrdersGet(datetime date_from, datetime date_to, MqlOrderInfoContainer& orders)
{
	...
	PyObject* py_args = PyTuple_New(2);
	PyObject* py_value = PyLong_FromUnsignedLongLong(date_from);
	PyTuple_SetItem(py_args, 0, py_value);
...
long MQL5::Metatrader5::HistoryDealsGet(datetime date_from, datetime date_to, MqlDealInfoContainer& deals_info)
{
	...
	deals_info.size = static_cast<long>(deals_count);
	...
	for (int i = 0; i < deals_count; ++i)
	{
		...
		info.magic = PyLong_AsLongLong(PyTuple_GetItem(py_order_tuple, 6));
		info.position_id = PyLong_AsLong(PyTuple_GetItem(py_order_tuple, 7));
...
long MQL5::Metatrader5::HistoryDealsGetBySymbol(datetime date_from, datetime date_to, const char* symbol, MqlDealInfoContainer& deals_info)
{
	...
	for (int i = 0; i < deals_count; ++i)
	{
		...
		info.magic = PyLong_AsLongLong(PyTuple_GetItem(py_order_tuple, 6));
		info.position_id = PyLong_AsLong(PyTuple_GetItem(py_order_tuple, 7));
		info.reason = (ENUM_DEAL_REASON)PyLong_AsLong(PyTuple_GetItem(py_order_tuple, 8));
		info.volume = PyFloat_AsDouble(PyTuple_GetItem(py_order_tuple, 9));
		info.price = PyFloat_AsDouble(PyTuple_GetItem(py_order_tuple, 10));
		info.commission = PyFloat_AsDouble(PyTuple_GetItem(py_order_tuple, 11));
		info.swap = PyFloat_AsDouble(PyTuple_GetItem(py_order_tuple, 12));
...
const std::string& MQL5::Metatrader5::CTrade::GetSymbol()
{
	return mSymbol;
}
...
void MQL5::Metatrader5::CTrade::SetTypeFilling(ENUM_ORDER_TYPE_FILLING type_filling)
{
...
bool MQL5::Metatrader5::CTrade::Buy(double tp, double sl, double volume, ulong deviation, const std::string& comment)
{
	...
	mComment = comment;
	mRequest.comment = (mComment.empty() ? nullptr : mComment.data());
...
bool MQL5::Metatrader5::CTrade::BuyStopLimit(double trigger_price, double limit_price, double tp, double sl, double volume, ulong deviation, const std::string& comment)
{
	mRequest.action = TRADE_ACTION_PENDING;
...
bool MQL5::Metatrader5::CTrade::SellStopLimit(double trigger_price, double limit_price, double tp, double sl, double volume, ulong deviation, const std::string& comment)
{
	mRequest.action = TRADE_ACTION_PENDING;
	mRequest.type = ORDER_TYPE_SELL_STOP_LIMIT;
	mRequest.deviation = deviation;
	mRequest.tp = tp;
	mRequest.sl = sl;
	mRequest.volume = volume;
...
bool MQL5::Metatrader5::CTrade::CancelPendingOrders(MqlTradeResult& res)
{
	ulong* orders=nullptr;
	long total_orders = 0;

	if ((total_orders = OrdersGetTickets(orders)) < 0)
	{
		return false;
	}
	mRequest.action = TRADE_ACTION_REMOVE;

	for (long i = 0; i < total_orders; ++i)
	{
		mRequest.order = orders[i];
...
bool MQL5::Metatrader5::CTrade::CancelPendingOrders(const char* symbol, MqlTradeResult& res)
{
	ulong* orders = nullptr;
	long total_orders = 0;

	if ((total_orders = OrdersGetTickets(symbol, orders)) < 0)
	{
		return false;
	}
	mRequest.action = TRADE_ACTION_REMOVE;
...
bool MQL5::Metatrader5::CTrade::ModifyPosition(ulong position_ticket, double tp, double sl)
{
...
bool MQL5::Metatrader5::CTrade::ClosePosition(ulong position_ticket, double volume)
{
	MqlPositionInfo pos;
	if (PositionsGetByTicket(position_ticket, pos) == false)
		return false;

	mRequest.action = TRADE_ACTION_DEAL;
	mRequest.position = position_ticket;
	mRequest.type = (pos.type == POSITION_TYPE_BUY ? ORDER_TYPE_SELL : ORDER_TYPE_BUY);
	mRequest.tp = 0;
	mRequest.sl = 0;
	mRequest.type_filling = ORDER_FILLING_IOC;
	mRequest.volume = (volume == 0 ? pos.volume : (volume > pos.volume ? pos.volume : volume));
...
std::string MQL5::Metatrader5::CTrade::ResultRetCodeAsStr()
{
	return MqlTradeResult::retCodeTable[mResult.retcode - 10004];
}
...
double MQL5::Metatrader5::CTrade::ResultBid()
{
	return mResult.bid;
}

double MQL5::Metatrader5::CTrade::ResultAsk()
{
	return mResult.ask;
}

double MQL5::Metatrader5::CTrade::ResultVolume()
{
	return mResult.volume;
}

double MQL5::Metatrader5::CTrade::ResultPrice()
{
	return mResult.price;
}

std::string MQL5::Metatrader5::CTrade::ResultComment()
{
	return mResult.comment;
}

uint MQL5::Metatrader5::CTrade::ResultRequestId()
{
	return mResult.request_id;
}

int MQL5::Metatrader5::CTrade::ResultRetCodeExternal()
{
	return *(int*)mResult.retcode_external;
}

const MqlTradeResult& MQL5::Metatrader5::CTrade::Result()
{
	return mResult;
}
...
void MQL5::Metatrader5::CTrade::pZeroResultComment()
{
	if (mResult.comment)
	{
		delete[] mResult.comment;
		mResult.comment = nullptr;
	}
}
